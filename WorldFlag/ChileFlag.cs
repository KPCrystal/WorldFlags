...
namespace WorldFlag
{
    public partial class ChileFlag : Form
    {
        /// <summary>
        /// 初期表示
        /// </summary>
        ...
        private void DrawFlag(Graphics g, float x0, float y0, float width)
        {
            ...
            // 青色の四角を作成する
            g.FillRectangle(blueBrush, x0, y0, width / 3, height / 2);
            // 白色の四角を作成する
            g.FillRectangle(whiteBrush, x0 + 2 * 1 * width / 6,
                y0, width - width/3, height / 2);
            // 赤色の四角を作成する
            ...
            {
                // Y Positionを設定
                float yc = y0 + offset + j * dy + dy / 12;
                for (int i = 0; i < 1; i++)
                {
                    // X Positionを設定
                    float xc = x0 + offset + i * dx + dx / 15;
                    if ((i + j) % 2 == 0)
                    {
                        DrawStar(g, this.Width / 15, xc, yc);
                    }
                }
            }
